...
function ($scope, $stateParams) {
    ...
    function respond(val) {
      ...
      if (val !== messageRecording) { //不是"讀取中"那段文字的化
		...
		switch (TestVal) {
            ...
            case "00015": //推薦系統
                var SearchKey = val.substr(5);
                Start00015(SearchKey);                
                val = "依照"+username+"的偏好，推薦您以下景點"+localStorage.getItem("Attraction");;
                msg.text = val;
                window.speechSynthesis.speak(msg);
                break;
            case "00016": //問自己是誰
    ...
    function Start00015(SearchKey) {
        var resp1;
        $.ajax({
            url: "http://120.119.164.95:5000/data/",
            type: "POST",
            data: {
            ...
            success: function(resp){
                localStorage.setItem("Attraction", "，一，"+resp.Attraction1+"，二，"+resp.Attraction2+"，三，"+resp.Attraction3+"，四，"+resp.Attraction4+"，五，"+resp.Attraction5);
            }
        });
    }
    ...
    registerSmtBtn.addEventListener("click",function(){
        if (pwd.value == pwd2.value) {
            firebase.auth().createUserWithEmailAndPassword(account.value, pwd.value).then(function(){
                console.log("成功註冊");      
                localStorage.setItem("LoginWay", "registered"); // 登入方式標記為註冊登入
